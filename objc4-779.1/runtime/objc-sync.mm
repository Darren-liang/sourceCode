...
typedef struct alignas(CacheLineSize) SyncData {
    struct SyncData* nextData;//指向下一个结构体SyncData的指针--链表存储的
    DisguisedPtr<objc_object> object;//这个就是@synchronized(obj),关联的括号里面的对象
    int32_t threadCount;  // number of THREADS using this block
    recursive_mutex_t mutex;//和object关联的递归锁
} SyncData;
