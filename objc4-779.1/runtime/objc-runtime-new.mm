...
**********************************************************************/
//在runtime初始化入口函数_objc_init里面加载map_images的时候，最终会走到这里
//这里会看到，吧category里面的方法，协议等都attach到类里面了，唯独没有处理属性，所以这也可以说明不能增肌属性，尽管category_t结构体里面有instanceProperties，这个只是声明，这里的实现根本没有处理属性，所以并不能增加属性
                ...
                if (cls->isStubClass()) {
                        ...
                        ||  cat->instanceProperties)
                    {
                        if (cls->isRealized()) {
                            ...
                            //objc::unattachedCategories.addForClass只是把类和category做一个关联映射
                            //而remethodizeClass才是真正去处理添加事宜的功臣
                            objc::unattachedCategories.addForClass(lc, cls);
                        }
                    }
                    //把类方法，协议，类属性attch到元类里面
...
// cls->data() === cls->class_data_bits_t->bits.data---》class_rw_t->(method_array_t)methods
