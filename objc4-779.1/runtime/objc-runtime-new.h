...
struct class_rw_t {
    ...
    const class_ro_t *ro;//里面存储的都是编译的类信息（只能读取不能写入）
    //你会发现这里没有ivars成员变量列表,因为成员变量都是在编译期就已经确定了的，不可以动态增加成员变量，在category增加属性也是通过关联对象实现的，并没有新增出成员变量，因此这里没有ivars，ivars都是在class_ro_t里面的，编译期直接确定了的
...
//objc_class里面没有发现isa，因为他是继承objc_object，objc_object里面有个isa，所以所有的objc_class都拥有一个isa
struct objc_class : objc_object {
    // Class ISA;
    Class superclass;//指向父类
    //方法缓存
    cache_t cache;             // formerly cache pointer and vtable
    //class_data_bits_t就是一个指针真正的方法存储在data()返回的指针指向的那块内存中。该内存其实是一个class_rw_t的类型值
